...
export default function EntryPage() {
  ...
  if (entry === null) {
    return (
      <div className="flex h-screen items-center justify-center text-[#BBBBB8]">
        Entry not found.
      </div>
    );
  }
    ...
    <AppShell>
      <main className="flex-1 overflow-y-auto">
        <div className="max-w-2xl mx-auto px-4 sm:px-6 py-5 sm:py-8">
          ...
          {/* Entry info */}
          <div className="rounded-xl border border-[#EAEAEA] bg-white p-6 mb-6">
            {editingEntry ? (
              ...
              <>
                <div className="flex items-start justify-between gap-2 mb-2">
                  <h1
                    className={cn(
                      "text-xl font-semibold text-[#111111]",
                      entry.status === "explored" && "line-through text-[#BBBBB8]"
                    )}
                  >
                    {entry.title}
                  </h1>
                  <button
                    ...
                    className="shrink-0 h-7 w-7 flex items-center justify-center rounded text-[#BBBBB8] hover:text-[#111111] hover:bg-[#F7F6F3] transition-colors"
                  >
                ...
                <a
                  ...
                  className="flex items-center gap-1.5 text-sm text-[#787774] hover:text-[#111111] hover:underline mb-4"
                >
                  <ExternalLink className="h-3.5 w-3.5" />
                  {getDomain(entry.url)}
                </a>
                {entry.notes && (
                  <div className="flex items-start gap-1.5 mb-4 text-sm text-[#787774] bg-[#F7F6F3] rounded-lg px-3 py-2">
                    <StickyNote className="h-3.5 w-3.5 mt-0.5 shrink-0 text-[#BBBBB8]" />
                    <p className="leading-relaxed">{entry.notes}</p>
                  </div>
                )}
              </>
            )}
            <div className="flex items-center gap-4 text-xs text-[#BBBBB8]">
          ...
          <div className="mb-4 flex items-center justify-between">
            <h2 className="text-base font-semibold text-[#111111]">
              ...
              {learnings && learnings.length > 0 && (
                <span className="ml-2 text-sm font-normal text-[#BBBBB8]">
                  ({learnings.length})
                </span>
              )}
            </h2>
            <Dialog open={addOpen} onOpenChange={setAddOpen}>
              ...
              <DialogContent>
                ...
                <form onSubmit={handleAddLearning} className="space-y-4">
                  ...
                  />
                  <div>
                    <p className="text-xs text-[#787774] mb-1.5">Rating (optional)</p>
          ...
          {learnings === undefined ? (
            ...
            <div className="text-center py-10 text-[#BBBBB8] border border-dashed border-[#EAEAEA] rounded-xl">
                  ...
                  {editingId === learning._id ? (
                    ...
                    <>
                      <div className="flex items-start justify-between gap-2">
                        ...
                        <div className="flex gap-1 shrink-0">
                          <button
                            ...
                            className="h-7 w-7 flex items-center justify-center rounded text-[#BBBBB8] hover:text-[#111111] hover:bg-[#F7F6F3] transition-colors"
                          >
                            <Pencil className="h-3.5 w-3.5" />
                          </button>
                          <button
                            onClick={() => removeLearning({ learningId: learning._id })}
                            className="h-7 w-7 flex items-center justify-center rounded text-[#BBBBB8] hover:text-red-500 hover:bg-red-50 transition-colors"
                          >
                      ...
                      <div className="flex items-center gap-3 mt-2">
                        ...
                        <span className="text-xs text-[#BBBBB8]">
      ...
      {[1, 2, 3, 4, 5].map((star) => (
        <button
          ...
          className={cn(
            ...
            star <= value ? "text-amber-400" : "text-[#EAEAEA] hover:text-amber-200"
          )}
        >
          ★
        </button>
